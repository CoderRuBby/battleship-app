...
describe('Turn', () => {
  ...
  let PlayerPlaceShips: PlaceShips;
  let NewTurn: Turn;

  beforeEach(() => {
    Player = new GameBoard();
    Opponent = new GameBoard();
    PlayerPlaceShips = new PlaceShips(Player);
    NewTurn = new Turn(Player, Opponent);
  });
  ...
  describe('turn', () => {
    it('will let player place ships if all ships have not been placed', () => {
      ...
      PlayerPlaceShips.selectShip(Battleship);
      console.log(PlayerPlaceShips.selectedShip);
      NewTurn.turn(square);
